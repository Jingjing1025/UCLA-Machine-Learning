...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            h = np.transpose(np.dot(X,self.coef_))
            self.coef_ = self.coef_ - 2 * eta * np.dot(h-y,X)

            # track error
            # hint: you cannot use self.predict(...) to make the predictions
            y_pred = np.transpose(np.dot(X,self.coef_)) # change this line
    ...
    def predict(self, X) :
        ...
        ### ========== TODO : START ========== ###
        # part c: predict y
        y = np.transpose(np.dot(X, self.coef_))
        ### ========== TODO : END ========== ###
    ...
    def cost(self, X, y) :
        ...
        # eta = np.array([0.0001, 0.001, 0.01, 0.0407])
        # eta = [0.03]
    ...
    def rms_error(self, X, y) :
        ...
        ### ========== TODO : START ========== ###
        # part h: compute RMSE
        error = np.power(self.cost(X,y)/float(X.shape[0]), 0.5)
        ### ========== TODO : END ========== ###
        return error
...
def main() :
    ...
    #     print model.cost(test_data.X, test_data.y)
    #     print "coef"
    #     print model.coef_
    #     print "\n"
    ...
    for i in range(11):
        ...
        model.fit(train_data.X, train_data.y)
        rmse[i, 0] = model.rms_error(train_data.X, train_data.y)
        rmse[i, 1] = model.rms_error(test_data.X, test_data.y)
        print "\n"
    plt.plot(m, rmse[:, 0], 'r.-', label = "Train Error")
    plt.plot(m, rmse[:, 1], 'b.-', label = "Test Error")
    plt.legend()
    plt.title("RMSE vs m")
    plt.xlabel("m")
    plt.ylabel("RMSE")
